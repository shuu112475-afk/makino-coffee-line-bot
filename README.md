...
Web上のデモ画面（`/`）から、LINEと同じ判定処理をその場で試せます。答えなかった質問は `/admin` の未対応キューに積まれ、担当者がその場で返信・FAQ反映まで行えます。
...
答えなかった質問は、担当者が個別に返信します。**この画面の出来がそのまま「答えない判断」のコストになる**ため、単なる入力欄にはしていません。

**判断材料をその場に置く。** 質問文だけを見せられても、担当者は「なぜ自動で答えられなかったのか」が分からず、毎回FAQ全体を読み直すことになります。理由（FAQの情報が不足 / 該当するFAQが無い / 障害で判定できず）、判定時に最も近かったFAQとその類似度、お客様との直前のやり取りを同じ画面に並べています。理由のラベルは状態ではなく**次にすべきこと**が分かる文言にしています。

**一覧性と作業領域を両立させる。** 1件の情報量を増やすと、溜まったときに全体像が掴めなくなります。一覧には理由・質問・近かったFAQだけを出し、作業領域は「対応する」で開きます。理由での絞り込みも付けましたが、**件数表示は絞り込みの影響を受けません**。「いま何が残っているか」はフィルタ中も見えている必要があるためです。

**誤送信を構造で防ぐ。** LINEのpushは取り消せません。送信前に確認を挟んだうえで、処理順を「**先に行を確保してから送る**」にしています。`status = 'pending'` を条件に付けたUPDATEは1つしか成功しないため、連打や複数人の同時対応でも同じ文面が2通届きません。送信に失敗した場合は確保を取り消して未対応に戻します。ここで解決済みのままにすると、**お客様に届かないまま画面から消えます**。

**人が返し続けないようにする。** 同じ質問が来るたびに人が返していては自動応答の意味がありません。対応した担当者がその場でFAQを追記・新規作成できる導線を置き、保存時に埋め込みを作り直して次回から自動で答えられる状態にします。近いFAQがある場合の既定は「追記」です。毎回新規に作ると似たFAQが増えて検索がぶれるためです。

送った文面は `/admin/history` から後から読み返せます。
...
- **管理画面から同じ文面を2通送らない**: 担当者の送信は「LINEへ送る前に行を確保する」順序で実装しています（`src/app/admin/actions.ts`）。詳細は[未対応キューの画面](#未対応キューの画面admin)を参照してください。
- **コネクションプーラー対応**: `POSTGRES_URL` はSupabaseのトランザクションモードのプーラー（6543番）を指すため、postgres.js の `prepare` を無効にしています。有効のままだと同時アクセス時に落ちます。

## コード変更後の再デプロイ

GitHubリポジトリをVercelに接続しているため、`main` への push がそのまま本番デプロイになります。プルリクエストにはプレビューURLが付きます。
...
`src/proxy.ts`（管理画面のBasic認証）を変更したときは、デプロイ後に**認証が通ることと、誤った資格情報が401になることの両方**を必ず確認してください。ここを壊すと管理画面に入れなくなるか、誰でも入れるようになります。
